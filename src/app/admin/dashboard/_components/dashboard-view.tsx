 ...
 *
 * `basePath` exists because /staff/dashboard renders this same view: it used to
 * re-export the admin page verbatim, so every card linked into /admin/... and
 * the admin layout bounced the staff member straight back out.
 */
export function DashboardView({ basePath = "/admin" }: { basePath?: string }) {
  ...
  const [pipeline, setPipeline] = useState<BillingPipeline | null>(null);
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        const [statsData, financialData, revenueData, activitiesData, queueData, pipelineData] =
          await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="O que precisa de você agora, e como anda a cobrança"
      />

      {/* Everything waiting on a decision comes first: this is the whole point
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Link href={`${basePath}/clients`} className="transition-transform hover:scale-[1.02]">
          <Card className="h-full cursor-pointer hover:border-primary/40 transition-colors">
        ...
        <Link href={`${basePath}/developments`} className="transition-transform hover:scale-[1.02]">
          <Card className="h-full cursor-pointer hover:border-primary/40 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Link href={`${basePath}/financial`} className="transition-transform hover:scale-[1.02]">
          <Card className="h-full cursor-pointer hover:border-primary/40 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Link href={`${basePath}/services`} className="transition-transform hover:scale-[1.02]">
          <Card className="h-full cursor-pointer hover:border-primary/40 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <QuickStat
          ...
          href={`${basePath}/clients?created=this_month`}
          accent="text-success"
        />
        <QuickStat
          icon={FileSignature}
